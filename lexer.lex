...
fun ctrl2 s =
  let
    val n = ord (valOf (Char.fromString s))
  in
    aHex n
  end
...
val palClaves = Polyhash.mkPolyTable(20, noEsta)
val _ = List.app (fn (c, v) => Polyhash.insert palClaves (c, v)) [
  ("type",        TYPE),
  ("array",       ARRAY),
  ("of",          OF),
  ("var",         VAR),
...
fun clavId s = (Polyhash.find palClaves s) handle noEsta => ID s
}
...
rule Tok = 
  parse "/*"            { inc ncom; Com lexbuf }
      ...
      | MN+             { clavId (getLexeme lexbuf) }
      | L LDU*          { ID (getLexeme lexbuf) }
      | _               { raise Fail ("lex:[" ^ (getLexeme lexbuf) ^ "]!") }

and literal = 
  parse eof             { raise Fail "string sin terminar!" }
      | `\n`            { raise Fail "nueva linea en string!" }
      | `"`             { "" }
...
and literal1 =
  parse eof             { raise Fail "string incompleta!" }
      | `\n`            { inc nlin; literal1 lexbuf }
      | SPC+            { literal1 lexbuf }
      | "\\"            { literal lexbuf }
      | _               { raise Fail "\\\\ malformada!" }

and Com = 
  parse "*/"            { (if dec ncom = 0 then Tok else Com) lexbuf }
      | "/*"            { inc ncom; Com lexbuf }
      | eof             { raise Fail "comentario incompleto!" }
      | `\n`            { inc nlin; Com lexbuf }
      | _               { Com lexbuf }
;
