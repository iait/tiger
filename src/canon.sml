...
(********************** linearize : stm -> stm list ***************************)
  (* produce una lista de sentencias que satisfacen:
   * 1- No hay SEQ ni ESEQ
   * 2- El padre de cada CALL es EXP(...) o MOVE(TEMP t, ...)
   *)
...
(**************** basicBlocks : stm list -> (block list * label) **************)
  (* Divide la lista de sentencias en bloques básicos donde cada bloque satisface:
   * 1- comienza con LABEL
   * 2- finaliza con JUMP o CJUMP
   * 3- no tiene LABEL, JUMP ni CJUMP en su interior
   *)
  fun basicBlocks stms = 
    let  val done = newLabel()
      ...
      | blocks(stms, blist) =
        blocks(LABEL(newLabel())::stms, blist)
    in (blocks(stms,nil), done) end

(****** traceSchedule : block list * temp.label -> tree.stm list *******)
  (* Ordena la lista de bloques de manera que se satisfaga:
   * 1- Cada CJUMP(cond, t, f) le sigue su LABEL(f)
   * 2- La mayor cantidad de JUMP(l) le siga LABEL(l)
   * Luego, se eliminan los JUMP y LABEL del caso 2 y se colectan todos 
   * los bloques en una lista de sentencias.
   *)
