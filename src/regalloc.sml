structure regalloc :> regalloc = struct
  ...
  fun regalloc (frm: frame) (body: instr list) =
    let
      (* COMPLETAR: Temporarios que ya tienen color asignado 
       * (p.ej, el temporario que representa a rax) *)
      val precolored = ["a", "b", "c", "d", "e", "f"]
      (* COMPLETAR: Temporarios que se pueden usar 
       * (p.ej, el temporario que representa a rax. 
       * Diferencia con precolored: el temporario que representa a rbp no se puede usar) *)
      val asignables = ["b", "c", "d", "e", "f"]
      (* COMPLETAR: movaMem crea una instrucción que mueve un temporario a memoria.
      ...
      fun rewriteInstr (OPER {assem, dst, src, jmp}) =
        let
          ...
          let
            fun gtc T [] = 
                  if Splayset.member(precoloredSet, T) then T 
          ...
          in
            ...
            map mksetMov (List.filter filterPC dst))
          end
