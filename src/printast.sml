  ...
  fun ppexpr pps e0 = 
    let
      ...
      and ppd (FunctionDec flist) = (
            ...
            List.app
              (fn ({name, params, result, body}, _) => (
                ...
                add_string pps "],";
                add_break pps (0, 1);
                add_string pps ("result="^(case result of SOME s => s | _ => "NONE"));
                add_break pps (0, 0);
                add_string pps "body="; 
                ppe body;
                add_string pps "}";
      ...
      |   ppd (VarDec ({name, escape, typ, init}, _)) = (
            ...
            add_string pps ",";
            add_break pps (0, 1);
            add_string pps ("typ="^(case typ of SOME s => s | _ => "NONE"));
            add_string pps ",";
            add_break pps (0, 1);
            add_string pps "init=";
            ppe init;
      ...
      and ppt (NameTy s) = (
        ...
        | ppt (ArrayTy s) = (
            begin_block pps INCONSISTENT 0;
            add_string pps ("ArrayTy("^s^")");
            add_break pps (0, 1);
            end_block pps)
  
      and ppv (SimpleVar s) = (
            begin_block pps INCONSISTENT 0;
            add_string pps "SimpleVar(";
            add_break pps (0, 1);
            add_string pps (s^")");
            add_break pps (0, 0);
            end_block pps)
        ...
        | ppv (SubscriptVar (v, e)) = (
            begin_block pps INCONSISTENT 0;
            add_string pps "SubscriptVar(";
            add_break pps (0, 1);
            ppv v; add_string pps ",";
            add_break pps (0, 1);
            ppe e;
            add_string pps ")";
            add_break pps (0, 0);
